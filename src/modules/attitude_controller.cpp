...
void AttitudeController ::control(float phi_r, float theta_r, float psi_r,
                                  ...
                                  float q, float r) {
  tau_phi = Ixx * control_siso(phi_r, phi, p, kp_phi, kd_phi);
  tau_theta = Iyy * control_siso(theta_r, theta, q, kp_theta, kd_theta);
  tau_psi = Izz * control_siso(psi_r, psi, r, kp_psi, kd_psi);
}
